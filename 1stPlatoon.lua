...
function onScriptTerminate(s, bool)
	if s == thisScript() and not bool then
		...
		sampAddChatMessage("{008B8B}[ 1]: {FFFAFA} .      (~)   .  CTRL + R  .", 0xFF008B8B)
	end
end
...
function checkupdate()
		...
		if tonumber(ver) > V then sampAddChatMessage("{008B8B}[ 1]: {FFFAFA}  ,   ...", 0xFF008B8B) updatescript(url, ver) end
end
...
function updatescript(url, ver)
	...
	sampAddChatMessage("{008B8B}[ 1]: {FFFAFA}  ,  :", 0xFF008B8B)
	sampAddChatMessage("{008B8B}[ 1]: {FFFAFA}" .. inf .. ",  ...", 0xFF008B8B)
